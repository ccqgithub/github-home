...
function Flatpickr(element, config) {
	...
	function bind() {
		...
		if (self.config.enableTime) {
			...
			self.sureContainer.addEventListener("click", function () {
				updateValue();
				if (self.selectedDates.length) triggerEvent("Change");
				if (self.selectedDates.length && self.config.mode === "single") self.close();
				return;
			});
	...
	function build() {
		...
		self.btmContainer = createElement("div", "flatpickr-btm flatpickr-clearfix");

		// ctrl
		self.sureContainer = createElement("a", "flatpickr-sure");
		self.sureContainer.innerHTML = '确定';

		self.btmContainer.appendChild(self.sureContainer);
		self.btmContainer.appendChild(buildTime());

		if (self.config.enableTime) fragment.appendChild(self.btmContainer);
	...
	function isEnabled(dateToCheck) {
		if (self.config.minDate && dateToCheck < self.config.minDate.fp_incr(0) || self.config.maxDate && dateToCheck > self.config.maxDate.fp_incr(0)) return false;
